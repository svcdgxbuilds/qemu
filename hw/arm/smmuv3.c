...
static void smmuv3_init_regs(SMMUv3State *s)
{
    ...
    /* vSMMU only supports nesting with one physical SMMU, so take the first */
    sdev = QLIST_FIRST(&bs->devices_with_nesting);
    if (sdev->info.idr[0] || !smmuv3_init_hw_regs(sdev)) {
                      ...
                      STE *ste, SMMUEventInfo *event)
{
    ...
    if (!bs->nested && STE_S1CDMAX(ste) != 0) {
        qemu_log_mask(LOG_UNIMP,
                      "SMMUv3 does not support multiple context descriptors yet\n");
        goto bad_ste;
    }

    if (!bs->nested && STE_S1STALLD(ste)) {
        qemu_log_mask(LOG_UNIMP,
