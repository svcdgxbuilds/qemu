                               ...
                               uint8_t tg, uint64_t num_pages, bool leaf)
{
    ...
    event.entry.granularity = IOMMU_INV_GRAN_ADDR;
    event.entry.granule = granule;
    event.entry.arch_id = asid;
    event.entry.flags = IOMMU_INV_FLAGS_GRANULE | IOMMU_INV_FLAGS_ARCHID;
    if (leaf) {
        event.entry.flags |= IOMMU_INV_FLAGS_LEAF;
    }
...
static void smmuv3_notify_cd_inv(SMMUState *bs, uint32_t sid, uint32_t ssid)
{
    ...
    event.entry.perm = IOMMU_NONE;
    event.entry.granularity = IOMMU_INV_GRAN_PASID;
    event.entry.flags = IOMMU_INV_FLAGS_PASID;
    event.entry.pasid = ssid;
...
static SMMUDevice *smmu_find_sdev(SMMUState *bs, uint32_t dev_id)
{
    SMMUDevice *sdev = NULL;

    QLIST_FOREACH(sdev, &bs->devices_with_notifiers, next) {
        if (!sdev->idev || sdev->idev->dev_id != dev_id)
		continue;
	break;
    }
...
static void smmuv3_inject_faults(void *smmu, struct iommu_fault *buf)
{
    ...
    SMMUEventInfo info = {};
    SMMUv3State *s3 = smmu;
    SMMUState *bs = ARM_SMMU(s3);
    SMMUDevice *sdev = smmu_find_sdev(bs, buf->dev_id);
...
static void smmuv3_config_ste(SMMUState *bs, uint32_t sid)
{
    ...
    struct iommu_user_arm_smmuv3 iommu_config = {};
    SMMUTransCfg *cfg;
    SMMUDevice *sdev;
    int ret;
    ...
    iommu_config.format = IOMMU_SMMU_FORMAT_SMMUV3;
    ...
    if (cfg->disabled || cfg->bypassed) {
        iommu_config.config = IOMMU_SMMU_CONFIG_BYPASS;
    } else if (cfg->aborted) {
        iommu_config.config = IOMMU_SMMU_CONFIG_ABORT;
    } else {
        iommu_config.config = IOMMU_SMMU_CONFIG_TRANSLATE;
    }

    trace_smmuv3_config_ste(mr->parent_obj.name,
                            iommu_config.config, cfg->s1ctxptr);

    ret = smmu_iommu_install_nested_ste(bs, sdev,
                                        &iommu_config, sizeof(iommu_config),
...
static struct iommu_user_arm_smmuv3 smmuv3_nested_data = {
                           ...
                           void *data)
{
    if (attr == IOMMU_ATTR_VFIO_NESTED) {
        IOMMUVFIONestedData nested_data = {
            .supported = true,
            .data_type = IOMMU_USER_ARM_SMMUV3,
            .data_len = sizeof(struct iommu_user_arm_smmuv3),
...
{
    SMMUDevice *sdev = container_of(iommu, SMMUDevice, iommu);
    return smmu_iommu_invalidate_cache(sdev,
