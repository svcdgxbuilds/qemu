...
void smmu_iommu_destroy_s1_hwpt(SMMUDevice *sdev)
{
    ...
    close(hwpt->fault_fd);
    iommufd_backend_free_id(hwpt->iommufd, hwpt->s2_hwpt_id);
                             ...
                             void (*fault_handler)(void *, struct iommu_fault *))
{
    ...
    ret = iommufd_backend_alloc_s2_hwpt(idev->iommufd, idev->dev_id,
                                        idev->ioas_id, &hwpt->s2_hwpt_id);
    if (ret) {
        error_report("Unable to allocate stage-2 HW pagetable: %d", ret);
        goto free_notifier;
    }

    ret = iommufd_backend_alloc_s1_hwpt(idev->iommufd, idev->dev_id, s1ptr,
                                        hwpt->s2_hwpt_id, hwpt->eventfd,
