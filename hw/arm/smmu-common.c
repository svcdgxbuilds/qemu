                                     ...
                                     IOMMUFDDevice *idev, Error **errp)
{
    ...
    if (QLIST_EMPTY(&s->devices_with_nesting)) {
        ...
        s->iommufd->listener = iommufd_s2hwpt_memory_listener;
        memory_listener_register(&s->iommufd->listener, &address_space_memory);
    }

    s2_hwpt = s->iommufd->s2_hwpt;
    ret = iommufd_device_attach_hwpt(idev, s2_hwpt->hwpt_id);
    if (ret) {
        if (QLIST_EMPTY(&s->devices_with_nesting)) {
...
int smmu_iommu_install_nested_ste(SMMUState *s, SMMUDevice *sdev,
                                  ...
                                  void *data)
{
    SMMUHwpt *hwpt = sdev->hwpt, *s2_hwpt;
    IOMMUFDDevice *idev;
    int ret;

    if (!s || !sdev || !s->iommufd || !s->iommufd->s2_hwpt) {
        return -ENOENT;
    }
    ...
    ret = iommufd_backend_alloc_hwpt(idev->iommufd->fd, idev->dev_id,
                                     s2_hwpt->hwpt_id, 0, data_type, data_len,
                                     data, &hwpt->hwpt_id);
    if (ret) {
                                ...
                                uint32_t *num, void *reqs)
{
    ...
    return iommufd_backend_invalidate_cache(idev->iommufd->fd, hwpt->hwpt_id,
                                            type, len, num, reqs);
}
...
static void smmu_base_realize(DeviceState *dev, Error **errp)
{
    ...
    if (s->iommufd) {
        ...
        if (*errp) {
            error_setg(errp, "failed to connect to IOMMUFD!");
            s->iommufd = NULL;
            if (s->nested) {
                error_setg(errp, "revert nested setup");
                s->nested = false;
            }
        }
    }
