...
static void vtd_init_stage1_config_data(struct iommu_hwpt_intel_vtd *vtd,
                                        VTDPASIDEntry *pe)
{
                           ...
                           uint32_t *s2_hwpt)
{
    ...
    ret = iommufd_backend_alloc_hwpt(idev->iommufd, 0, idev->dev_id,
                                     IOMMU_HWPT_TYPE_S2, idev->ioas_id,
                                     IOMMU_HWPT_DATA_NONE, NULL, 0, s2_hwpt);
    if (!ret) {
                            ...
                            IOMMUFDDevice *idev, VTDPASIDEntry *pe)
{
    ...
    ret = iommufd_backend_alloc_hwpt(idev->iommufd, IOMMU_HWPT_FLAG_NESTING,
                                     idev->dev_id, IOMMU_HWPT_TYPE_S1,
                                     s2_hwptid, IOMMU_HWPT_DATA_INTEL_VTD,
                                     &vtd, sizeof(vtd), &hwpt_id);
    if (ret) {
        vtd_put_s2_hwpt(s);
        event_notifier_cleanup(n);
        return ret;
    ...
    ret = iommufd_backend_add_hwpt_event(idev->iommufd, idev->dev_id, hwpt_id,
                                         fd, &fault_data_fd);
    if (ret) {
                                ...
                                gpointer user_data)
{
    ...
    vtd_idev = vtd_bus->idevs[vtd_pasid_as->devfn];
                                            ...
                                            VTDPASIDCacheInfo *pc_info)
{
    ...
    QLIST_FOREACH(vtd_idev, &s->vtd_idev_list, next) {
        ...
        walk_info.vtd_bus = vtd_idev->vtd_bus;
                                  ...
                                  struct iommu_cache_invalidate_info *cache)
{
    ...
    if (iommufd_backend_invalidate_cache(hwpt->iommufd, hwpt->hwpt_id, cache)) {
                                     ...
                                     IOMMUFDDevice *idev)
{
    ...
    vtd_bus->idevs[devfn] = vtd_idev =
                    g_malloc0(sizeof(VTDIOMMUFDDevice));
    vtd_idev->vtd_bus = vtd_bus;
