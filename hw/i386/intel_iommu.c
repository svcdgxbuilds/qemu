                                   ...
                                   uint32_t level, vtd_page_walk_info *info)
{
    ...
    while (iova < end) {
        ...
        if (!vtd_is_last_flpte(flpte, level) && vtd_flpte_present(flpte)) {
            ...
            level--;
            vtd_flt_page_walk_level(addr, iova, MIN(iova_next, end),
                                    level, info);
        } else {
            event.type = IOMMU_NOTIFIER_UNMAP;
            event.entry.target_as = &address_space_memory;
                                            ...
                                            uint32_t *s2_hwptid)
{
    ...
    ret = iommufd_backend_alloc_ioas(iommufd, &ioas_id, &err);
    if (ret < 0) {
...
static void vtd_put_ioas_s2_hwpt_with_errata(IOMMUFDDevice *idev)
{
    ...
    iommufd_backend_free_id(iommufd, s2_hwpt->hwpt_id);
    iommufd_backend_free_id(iommufd, s2_hwpt->parent_id);
    g_free(s2_hwpt);
...
static void vtd_put_s2_hwpt(IOMMUFDDevice *idev)
{
    ...
    iommufd_backend_free_id(idev->iommufd, idev->parent_hwpt);
    idev->parent_hwpt = 0;
}
                                   ...
                                   uint32_t rid_pasid)
{
    ...
    trace_vtd_device_attach_hwpt(vtd_pasid_as->pasid, hwpt->hwpt_id, ret);
    if (ret) {
        if (vtd_pe_pgtt_is_flt(pe)) {
                                  ...
                                  uint32_t rid_pasid)
{
    ...
    if (vtd_pasid_as->iommu_state->dmar_enabled) {
        ...
        trace_vtd_device_detach_hwpt(vtd_pasid_as->pasid, hwpt->hwpt_id, ret);
    } else {
        ret = iommufd_device_attach_hwpt(idev, idev->def_hwpt_id);
        trace_vtd_device_reattach_def_hwpt(vtd_pasid_as->pasid,
                             ...
                             Error **errp)
{
    ...
    if (s->cap_finalized) {
        bool ret = vtd_check_hw_info(s, vtd);
        if (!ret) {
            error_setg(errp, "vIOMMU cap/ecap does not compatible with host");
        }
        return ret;
    }
