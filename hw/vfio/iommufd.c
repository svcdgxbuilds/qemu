                          ...
                          uint32_t hwpt_id, Error **errp)
{
    ...
    if (ret) {
        ret = -errno;
        error_setg_errno(errp, -ret, "detach %s from hwpt_id=%d failed",
                         vbasedev->name, hwpt_id);
    }
    return ret;
}
...
__vfio_device_detach_container(VFIODevice *vbasedev,
                               ...
                               Error **errp)
{
    __vfio_device_detach_hwpt(vbasedev, container->iommufd, hwpt_id, errp);
...
out:
    ...
    hwpt = vfio_find_hwpt_for_dev(container, vbasedev);
    iommu_device_init(idev, sizeof(*idev), TYPE_VFIO_IOMMU_DEVICE,
                      container->iommufd, vbasedev->devid, hwpt->hwpt_id);
...
static int vfio_iommu_device_attach_stage1(IOMMUFDDevice *idev,
                                           uint32_t *pasid,
                                           uint32_t hwpt_id)
{
    VFIODevice *vbasedev = container_of(idev, VFIODevice, idev);
    VFIOIOMMUFDContainer *container =
        container_of(vbasedev->container, VFIOIOMMUFDContainer, obj);
    struct vfio_device_attach_hwpt attach = {
        .argsz = sizeof(attach),
        .flags = pasid ? VFIO_DEVICE_ATTACH_PASID : 0,
        .iommufd = container->iommufd,
        .hwpt_id = hwpt_id,
        .pasid = pasid ? *pasid : 0,
    };
...
static int vfio_iommu_device_detach_stage1(IOMMUFDDevice *idev,
                                           uint32_t hwpt_id)
{
    VFIODevice *vbasedev = container_of(idev, VFIODevice, idev);
    VFIOIOMMUFDContainer *container =
    ...
    ret = __vfio_device_detach_hwpt(vbasedev, container->iommufd, hwpt_id, &err);
    error_free(err);
    return ret;
}
                                         ...
                                         void *data)
{
    ...
    idevc->attach_stage1 = vfio_iommu_device_attach_stage1;
    idevc->detach_stage1 = vfio_iommu_device_detach_stage1;
}

static const TypeInfo vfio_iommu_device_info = {
    .parent = TYPE_IOMMU_DEVICE,
