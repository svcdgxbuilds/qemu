                                        ...
                                        Error **errp)
{
    ...
    if (bcontainer->nested.supported) {
        struct vfio_device_attach_hwpt attach = {
            .argsz = sizeof(attach),
            .iommufd = bind.iommufd,
        };

        /* Allocate and attach device to an S2 hwpt */
        ret = iommufd_backend_alloc_hwpt(bind.iommufd, IOMMU_HWPT_FLAG_NESTING,
                                         vbasedev->devid, IOMMU_HWPT_TYPE_S2,
                                         container->ioas_id,
                                         bcontainer->nested.data_type,
                                         bcontainer->nested.data,
                                         bcontainer->nested.data_len, &hwpt_id);
        if (ret) {
        ...
        hwpt_id = attach_data.out_hwpt_id;
    }
                                 ...
                                 Error **errp)
{
    ...
    if (memory_region_is_iommu(as->root)) {
        ...
        if (ret) {
            bcontainer->nested.supported = false;
            bcontainer->nested.data_type = IOMMU_HWPT_DATA_NONE;
            bcontainer->nested.data_len = 0;
            bcontainer->nested.data = NULL;
        }
        trace_vfio_iommufd_nested(vbasedev->iommufd->fd,
                                  bcontainer->nested.supported,
                                  bcontainer->nested.data_type,
                                  (uint64_t)bcontainer->nested.data);
    }
